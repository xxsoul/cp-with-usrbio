...
impl Iov {
    pub fn wrap(mountpoint: &str, shm: &Shmem, numa: i32) -> Result<Self, i32> {
        ...
        // 获取共享内存ID（可能是 "/cp_usrbio_xxx" 或 "cp_usrbio_xxx"）
        let os_id = shm.get_os_id();

        // 构建目标路径（共享内存文件的完整路径）
        // 如果 os_id 以 / 开头，直接使用；否则添加 /dev/shm/ 前缀
        ...
        // 创建符号链接
        fs::symlink(&targ, &link)
            .map_err(|e| {
                eprintln!("Failed to create symlink: {} -> {}", targ, link);
                eprintln!("Error: {}", e);
                -e.raw_os_error().unwrap_or(22)
            })?;
...
impl Drop for Iov {
    fn drop(&mut self) {
        unsafe {
            hf3fs_iovdestroy(&mut self.0 as *mut _);
        }
    }
}
